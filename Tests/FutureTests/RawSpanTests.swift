...
//    )
  ...
  func testInitWithRawBytes() {
    ...
    a.withUnsafeBytes {
      let span = RawSpan(_unsafeBytes: $0, owner: a)
      XCTAssertEqual(span.byteCount, capacity*MemoryLayout<Int>.stride)
    }

    a.withUnsafeMutableBytes {
      let span = RawSpan(_unsafeBytes: $0, owner: $0)
      XCTAssertEqual(span.byteCount, capacity*MemoryLayout<Int>.stride)
    }
  }
...
//    _ = rs2
  ...
  func testPrefix() {
    ...
    do {
      ...
      let span = RawSpan(_unsafeBytes: b, owner: b)
  ...
  func testContainment() {
    ...
    let fakeSpan = RawSpan(
      _unsafeStart: b.baseAddress!.advanced(by: 8), byteCount: 8, owner: b
    )
    let nilSpan = RawSpan(
      _unsafeBytes: UnsafeRawBufferPointer(start: nil, count: 0), owner: b
    )
