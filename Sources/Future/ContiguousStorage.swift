  ...
  public var storage: Span<Element> {
    _read {
      if let a = _baseAddressIfContiguous {
        yield Span(_unsafeStart: a, count: count, owner: self)
      }
      else {
        let a = ContiguousArray(copy self)
        #if true
        let s = Span(
          _unsafeStart: a._baseAddressIfContiguous!, count: a.count, owner: a
        )
        #else
        let s = a.storage
        #endif
...
extension ContiguousArray: ContiguousStorage {
  public var storage: Span<Element> {
    borrowing get {
      Span(
        _unsafeStart: _baseAddressIfContiguous!, count: count, owner: self
      )
    }
  }
}
...
*/
      ...
      yield Span(
        _unsafeStart: a._baseAddressIfContiguous!, count: 1, owner: a
      )
    }
  }
}
...
//        yield a.storage
        ...
        yield Span(_unsafeElements: buffer, owner: self)
      }
